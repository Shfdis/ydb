        ...
        void UpdateTicket() {
            ...
            auto cb = [self = this->shared_from_this(), response] (grpc::Status status) mutable {
                ...
                try {
                    ...
                    dispatchError = CurrentExceptionMessage();
                }
                ...
                self->Fail(TStringBuilder()
                    << "IAM-token provider response facility is not available"
                    << (dispatchError.empty() ? "" : ": ") << dispatchError);
            };
            ...
            try {
                RequestFiller_(req);
                if (!Context_) {
                    throw yexception() << "IAM-token request context is not available";
                }
